...
const BETWEEN_TINT = '#FFF5F5';
const DATA_DOT_GRAY = '#9CA3AF';
  ...
  const renderItem = useCallback(
    ({ item }: { item: Date }) => {
      ...
      const cellStyle = isEndpoint
        ? [styles.cellActive, { backgroundColor: C.red }]
        : between
          ? [styles.cellBetween, { backgroundColor: BETWEEN_TINT, borderColor: C.redBorder }]
          : [styles.cellInactive, { backgroundColor: C.redBg, borderColor: C.redBorder }];

      // Dot: between = red; endpoint w/ data = white; unselected w/ data = gray.
      let dotColor: string | null = null;
      if (between) dotColor = C.red;
      else if (isEndpoint) dotColor = hasData ? '#FFF' : null;
      else if (hasData) dotColor = DATA_DOT_GRAY;

      return (
        <TouchableOpacity activeOpacity={0.8} onPress={() => handlePress(item)} style={[styles.cell, ...cellStyle]}>
          <Text style={[styles.dayLabel, { color: isEndpoint ? 'rgba(255,255,255,0.85)' : C.textSm }]}>
            {DAY_ABBR[item.getDay()]}
          </Text>
          <Text
            style={[
              styles.dayNum,
              { color: isEndpoint ? '#FFF' : C.text },
