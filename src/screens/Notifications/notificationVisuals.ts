...
export const cardIcon = (item: AlertItem, c: AppTheme): CardIcon => {
  ...
  if (sev === 'critical') {
    return { Icon: item.tag === 'System' ? AlertTriangle : Zap, circle: c.primary, color: c.textOnPrimary };
  }
  if (sev === 'warning') {
    return { Icon: /variab|fluctuat|unusual/.test(t) ? Activity : TrendingUp, circle: c.criticalBg, color: c.criticalText };
  }
  if (/log|meal|breakfast|lunch|dinner|carb|oatmeal|snack/.test(t)) return { Icon: Utensils, circle: c.warningBg, color: c.warningText };
  if (/normal|in range|target|good|stable/.test(t)) return { Icon: CheckCircle2, circle: c.loggedTagBg, color: c.loggedTagText };
  if (/activity|exercise|walk|step|run/.test(t)) return { Icon: Activity, circle: c.infoBg, color: c.infoText };
  return { Icon: Bell, circle: c.backgroundMuted, color: c.textMuted };
};
